...
# 音声トリム（先頭・末尾の無音除去）設定
# TTS が出力する WAV の余白を削り、字幕とセリフのズレを縮める
TRIM_SILENCE_ENABLED = True
TRIM_THRESHOLD_DB = -40.0      # この値以下を無音とみなす（speech 検出と同条件）
TRIM_HEAD_PAD_SEC = 0.05       # 検出した speech 開始位置の前にこの秒数だけ無音を残す
TRIM_TAIL_PAD_SEC = 0.10       # 検出した speech 終了位置の後にこの秒数だけ無音を残す
...
TONE_TAG_MAP: dict[str, str] = {
    ...
    "憂鬱": "tired",
    "決意": "determined",
    "笑い": "with a light laugh",
    "ため息": "with a soft sigh",
    "ささやき": "in a quiet whisper",
}

# キャラクター別 TTS スタイル定義（Gemini TTS への自然言語プレフィックス）
CHAR_STYLE_PREFIX: dict[str, str] = {
    "NAGISA":   "You are Nagisa, a gentle and introspective AI. Speak softly and thoughtfully. Avoid over-acting.",
    "SHINJI":   "You are Shinji, a warm-hearted 16-year-old boy. Sound natural and youthful, not theatrical.",
    "NARRATOR": "You are a calm, professional narrator. Use clear and measured diction.",
    "SYSTEM":   "You are a calm AI system voice. Speak evenly and clearly.",
}
