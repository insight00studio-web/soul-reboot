...
def check_claude_token() -> tuple[bool, str]:
    """Claude OAuth トークンの有効性チェック（Anthropic API を疎通確認）"""
    token = os.environ.get("CLAUDE_CODE_OAUTH_TOKEN", "")
    if not token:
        return False, "CLAUDE_CODE_OAUTH_TOKEN が設定されていません"
    ...
    try:
        ...
        if e.code == 401:
            return False, "トークン無効（401 Unauthorized）: update_token.ps1 を実行してください"
        return True, f"API応答 {e.code}（トークン自体は有効の可能性）"
    except Exception as e:
        return False, f"疎通確認エラー: {e}"
