...
3. **画像プロンプト**: Gemini Image Generation用の自然言語英語プロンプトを指定。**キャラクター一貫性のため以下のルールを厳守**:
    ...
    - **服装ルール（絶対厳守）**: プロンプト内「【最重要】服装指定」に記載された服装を全シーンに統一すること
...
8. **字幕文字数制限（必須）**: `line_text` は**全角25文字以内**を目安にすること。動画の字幕バーに2行以内で収める必要がある。
   - 長いセリフは自然な区切り（句読点・接続詞）で**複数の `line_text` に分割**すること
   - NG例: 「あなたがそんなこと言うなんて、正直驚いています。でも、それが本当の気持ちなのなら、私は受け入れなければなりません」（長すぎ）
   - OK例: 「あなたがそんなことを言うとは。」→ 次行 「…正直、驚いています。」→ 次行 「でも、本当の気持ちなら受け入れます。」
